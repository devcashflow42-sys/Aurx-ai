...
/* ══════════════════════════════════════════════
   SESSION — guarda datos del usuario (sin token)
   El token viaja como cookie httpOnly (el browser
   lo envía automáticamente; JS nunca puede leerlo)
══════════════════════════════════════════════ */
function saveSession(data) {
  // Non-sensitive flag so the chat page knows a session exists
  localStorage.setItem('aurx_session', '1');
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  // Si hay sesión local → verificar con el servidor.
  // Solo redirigimos si el servidor confirma que el token sigue válido.
  // Esto evita bucles: si el token expiró, limpiamos los flags y el
  // usuario puede iniciar sesión normalmente.
  if (localStorage.getItem('aurx_session')) {
    fetch(API_BASE + '/api/users/profile', { credentials: 'include' })
      ...
      .catch(function () {}); // error de red → quedarse en login
  }
});
