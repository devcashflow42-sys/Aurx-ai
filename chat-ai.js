...
(function () {
  ...
  btnNew.addEventListener('click', () => {
    chatActive = false; messages.innerHTML = ''; messages.classList.remove('active');
    hero.style.display = ''; input.value = ''; input.style.height = 'auto';
    clearAllFiles(); updateSendBtn(); closeSidebar();
  ...
  function getToken() {
    return localStorage.getItem('aurx_token') || null;
  }
  ...
  function logout() {
    localStorage.removeItem('aurx_token');
  ...
  function sendMessage() {
    ...
    .then(function (data) {
      ...
      if (!data) return;
      if (!data.success) {
        addMsg('ai', '⚠️ ' + (data.message || 'Error al obtener respuesta.'), []);
        return;
      }
      addMsg('ai', data.data.response, []);
    })
    .catch(function () {
      removeTyping(typingId);
      addMsg('ai', '⚠️ Error de conexión. Verifica tu red e intenta de nuevo.', []);
    });
  }
